...
=head1 TRAVERSING THE OBJECT HIERARCHY

We can use the very awesome Data::Dumper to dump the hierarchy.

    use Data::Dumper;
    
    print Dumper($loader->root_object);

Ewwww. That output is horribly ugly, and Perl is above horribly ugly.
...
BinderItems.  Let's get the root Binder, then:
    ...
    my $binder = $loader->root_object->Binder_collection;

Well, actually, that's silly. We don't have a collection of Binders.
But that'll do for now. See "OPTIMIZING THE HIERARCHY" for more
information.

=head1 OPTIMIZING THE HIERARCHY
...
XXX see http://gist.github.com/2981483
